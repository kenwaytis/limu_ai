...
def gpu_evaluate_accuracy(net, data_iter):
    """Compute the accuracy for a model on a dataset.
...
def gpu_train_epoch_ch3(net, train_iter, loss, updater):
    """The training loop defined in Chapter 3.
...
def gpu_train_ch3(net, train_iter, test_iter, loss, num_epochs, updater):
    """Train a model (defined in Chapter 3).
    ...
    animator = Animator(xlabel='epoch', xlim=[1, num_epochs], ylim=[0.3, 0.9],
                        legend=['train loss', 'train acc', 'test acc'])
    for epoch in range(num_epochs):
        train_metrics = gpu_train_epoch_ch3(net, train_iter, loss, updater)
        test_acc = gpu_evaluate_accuracy(net, test_iter)
        animator.add(epoch + 1, train_metrics + (test_acc,))
    train_loss, train_acc = train_metrics
    assert train_loss < 0.5, train_loss
    assert train_acc <= 1 and train_acc > 0.7, train_acc
...
def train_ch6(net, train_iter, test_iter, num_epochs, lr, device):
    ...
    for epoch in range(num_epochs):
        count_epochs += 1
        print('***in epoch:\t' + str(count_epochs))
        # Sum of training loss, sum of training accuracy, no. of examples
